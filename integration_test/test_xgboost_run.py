...
POSSIBLE_PARAMETERS = {
  ...
  "tree_method": secrets.SystemRandom().choice(["hist", "exact", "approx", "auto"]),
}
...
def _create_random_metric_objective(task="binary"):
  if task == "binary":
    return {
      "objective": secrets.SystemRandom().choice(["binary:logistic", "binary:hinge", "binary:logitraw"]),
      "eval_metric": ["logloss", "aucpr", "error"],
    }
  elif task == "multiclass":
    return {
      "objective": secrets.SystemRandom().choice(["multi:softmax", "multi:softprob"]),
      "eval_metric": ["mlogloss", "merror"],
    }
  else:
    return {
      "objective": secrets.SystemRandom().choice(["reg:squarederror", "reg:pseudohubererror"]),
...
def _form_random_run_params(task):
  ...
  return dict(
    ...
    verbose_eval=secrets.SystemRandom().choice([True, False]),
