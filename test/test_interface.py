...
class TestInterface(object):
  ...
  def test_create(self):
    conn = Connection(client_token='client_token', _show_deprectation_warning=False)
  ...
  def test_create_uses_session_if_provided(self):
    session = mock.Mock()
    conn = Connection(client_token='client_token', session=session, _show_deprectation_warning=False)
  ...
  def test_environment_variable(self):
    with mock.patch.dict(os.environ, {'SIGOPT_API_TOKEN': 'client_token'}):
      conn = Connection(_show_deprectation_warning=False)
      assert conn.impl.requestor.auth.username == 'client_token'
  ...
  def test_api_url(self):
    conn = Connection('client_token', _show_deprectation_warning=False)
  ...
  def test_error(self):
    with mock.patch.dict(os.environ, {'SIGOPT_API_TOKEN': ''}):
      with pytest.raises(ValueError):
        Connection(_show_deprectation_warning=False)
